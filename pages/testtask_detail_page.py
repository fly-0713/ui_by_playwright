...
class TesttaskDetailPage(BasePage):
    """测试任务详情页面对象"""

    def __init__(self, page: Page):
        super().__init__(page)
        # 左侧菜单
        self._menu_test_work = page.get_by_text("测试作业", exact=True)
        self._menu_test_task = page.get_by_text("测试任务")
        # 搜索区域
    ...
    def navigate_to_test_task(self):
        """从首页导航到测试任务页面"""
        logger.info("导航到测试任务页面")
        self._menu_test_work.click()
        self.page.wait_for_timeout(500)
        self._menu_test_task.click()
        self.page.wait_for_timeout(1000)
        logger.info(f"已进入测试任务页面, URL: {self.page.url}")
    ...
    def get_test_serial_number(self) -> str:
        """从详情页获取测试序列号
        ...
        if col_index != -1:
            ...
            test_serial = cell.inner_text().strip()
        else:
            # fallback：获取第一个包含 TEST 的文本
            test_serial = self.page.locator("text=/TEST\\d+/").first.inner_text().strip()

        logger.info(f"获取到测试序列号: {test_serial}")
        return test_serial

    def get_test_station_code(self) -> str:
        """从详情页获取测试工位编码

        Returns:
            测试工位编码字符串
        """
        logger.info("获取测试工位编码")
        # 通过表格列名获取
        col_index = self._get_column_index("测试工位")
        if col_index != -1:
            cell = self.page.locator(
                f".el-table__body-wrapper tbody tr:first-child td:nth-child({col_index})"
            )
            ...
            # fallback：获取第一个包含 CGW 的文本
            station_code = self.page.locator("text=/CGW\\d+/").first.inner_text().strip()

        logger.info(f"获取到测试工位编码: {station_code}")
        return station_code

    def click_product_code(self, product_code: str):
        """点击产品号

        Args:
            product_code: 产品号文本
        """
        logger.info(f"点击产品号: {product_code}")
        self.page.get_by_text(product_code).first.click()
